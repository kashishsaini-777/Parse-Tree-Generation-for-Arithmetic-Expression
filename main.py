...
# main.py
from lexer import tokenize
from bottomup_parser import BottomUpParser   # ✅ use the new bottom-up parser
from visualizer import visualize_tree

def main():
    print("🚀 Arithmetic Expression Parse Tree Generator (Bottom-Up)")
    print("Enter an expression (e.g., '3 * (4 + 2)') or type 'exit' to quit.")
    ...
    while True:
        try:
            ...
            # 2. Syntax Analysis (Parsing)
            parser = BottomUpParser(tokens)  # ✅ swapped parser
            parse_tree = parser.parse()
